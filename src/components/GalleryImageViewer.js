...
export default class GalleryImageViewer extends Component {
                ...
                case 'success':
                    return (
                        <View style={viewer.loadedImg} key={index}>
                            <Animated.View
                                style={{
                                    width: width, height: height, transform: [
                                        { scale: scalable },
    ...
    render() {
        ...
        return (
            <Modal visible={shown} transparent={true} animationType={"none"} onRequestClose={this.modalDismissed.bind(this)} >
                ...
                <Animated.View style={[viewer.bottomBar, {
                    opacity: this.state.fadeAnim
                }]}>

                    <Textarea rowSpan={3} bordered placeholderTextColor='#fff' value={this.state.curDescription} placeholder="Digite aqui a descrição da foto..." ref={(el) => { this.curDescription = el; }} onChangeText={(curDescription) => {
    ...
    init(props) {
        ...
        this.setState({
            curIndex: index,
            curDescription: this.props.imageUrls[index].description,
            maxIndex: len - 1,
            midIndex: Math.floor((len - 1) / 2),
            imagesInfo: temp,
            auxList: this.props.imageUrls,
            imageLoaded: false
        }, () => {
